	...
	mounted(){
		...
		if (state == 'zjzxQQAuthorization') {
			let appid = 101532474,
				...
				access_token = hash.substring(hash.indexOf('=') + 1,hash.indexOf('&'));
				...
				$.ajax({
					...
					success(data){
						...
						$.post("http://47.101.58.43:8185/three/getThreeInfo",params,function(info){
							...
							// userService.loginByQQ(userparams,this.userInfoStore)
						});
					}
				})
			}
		...
		if (state == 'zjzxWechatAuthorization') {
			...
			$.post("http://47.101.58.43:8185/three/getThreeInfo",params,function(data){
				...
				$.post('http://47.101.58.43:8185/three/getThreeInfo',_params,function(info){
					...
					console.log(userparams)
					// userService.loginByWx(userparams,this.userInfoStore)
				})
			});
			return;
		}
		 ...
		 * 通过code获取access_token
		 * https://api.weibo.com/oauth2/access_token?client_id=273153298&client_secret=88090437f53685a2c4ea967a9f65af7a&grant_type=authorization_code&redirect_uri=http%3A%2F%2Fwww.zjzx.xyz%2F%23%2Fmember&code=3ffce78a8d582ff8f9401560efc46c5a
		 */
		if (state == 'zjzxSinaAuthorization') {
			...
			let _param = {
				...
				'grant_type':'authorization_code',
				'redirect_uri':encodeURIComponent('http://www.zjzx.xyz/#/member'),
			...
			console.log(_params)
			$.post("http://47.101.58.43:8185/three/getThreeInfo",_params,function(data){
			debugger
				let param = {
					access_token:data.access_token,
					"uid":data.uid
